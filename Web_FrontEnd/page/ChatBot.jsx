...
function ChatBot() {
  ...
  const hyperlinkText = (text) => {
    const urlRe = /^http:\/\/localhost(:\d+)?(\/\S*)?/g;  // localhost URL이 있는지 확인하는 정규식
    if (urlRe.test(text)) {
      return  `<a href="${text}" >${text}</a>`; // 메시지 전체를 하이퍼링크로 변환  // target="_blank" = 새창
    }
    return text
  }
    ...
    if (chat.trim()) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.data) {
            const linkMessage = hyperlinkText(data.data);
            setChatList(prevChatList => [{ text: linkMessage, id: data.newChatId, type: 'bot' }, ...prevChatList]);
          }
        } else {
                  ...
                  return (
                    <div key={index} className={`ChatBot-message-container ${ct.type === 'user' ? 'user' : 'bot'}`}>
                      ...
                      {ct.type === 'bot' && (
                        ...
                        >
                          <img src={speaker} alt="Bot Avatar" className="ChatBot-speaker-image" />
                        </button>
                      )}
                    </div>
                  );
