...
def finance_pattern_query(finance_query, input_time=None, entity1=None, entity2=None, date_query=None, text=None):
    ...
    if finance_query == "지출" or finance_query == "소득" or r'구매|구입|\b산\b' in no_space_text:
        ...
        detail_conditions = {
            ...
            "저축": "(rp_detail = '예금' OR rp_detail = '적금')",
            "저금": "(rp_detail = '예금' OR rp_detail = '적금')",
        }
        ...
        query["링크"] = "http://localhost:3000/myassetplaner"
        ...
        for _ in range(len(entity1)):
            if "상환" in text:
                ...
                query['대출상환'] = f'SELECT uf.user_id, uf.uf_loan, rp.rp_date, rp.rp_detail, rp.rp_amount FROM tb_user_finance uf JOIN tb_received_paid rp ON uf.user_id = rp.user_id WHERE rp.rp_detail = "대출 상환" AND uf.user_id={user_id}' + " " + date_query
...
current_dir = r"C:\[jwkim]\0_project\aicc_contest\aicc_map\server\src\Algorithm\script"
...
# 증시
def market_news_answer():
    with open('stokc_market_news.txt', mode='r') as market_news:
        return market_news.read()
        ...
        try:
            ...
            # 두 번째 입력 처리: key와 query_result가 있는 경우
                ...
                try:
                    ...
                    # elif any(i in keyword for i in ['증시', '뉴스']):
                    #     sentence = market_news_answer()
                    #     print(sentence, flush=True)
                    else:
