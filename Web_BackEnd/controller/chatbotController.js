...
const postChatbotData = async (req, res) => {
  ...
  try {
    ...
    // Python 실행
    const return_query = spawn('C:\\Users\\4\\anaconda3\\Scripts\\conda.exe', ['run', '-n', 'projectM4', 'python', returnQueryPath, message, user_id]);
    ...
    return_query.on('close', async (code) => {
      ...
      try {
        ...
        for (const [key, query] of Object.entries(parsedData)) {
          ...
          const sentence_creation = spawn('C:\\Users\\4\\anaconda3\\Scripts\\conda.exe', ['run', '-n', 'projectM4', 'python', sentenceCreationPath, sentence_data, sentence_key]);
          ...
          sentence_creation.on('close', async (code) => {
            ...
            if (key === Object.keys(parsedData).pop()) {
              ...
              // 최종 응답 전송
              res.json({ data: finalResult.trim() });
            }
          });
        }
      } catch (error) {
...
const getChatDetail = async (req, res) => {
  ...
  try {
    ...
    if (result.length > 0) {
      res.json(result[0]); // cb_text 반환
    } else {
      res.status(404).json({ error: 'Not Found' });
    }
  } catch (error) {
    console.error('에러 코드: DB_004 - 채팅 상세 내용 가져오기 실패:', error);
    res.status(500).json({ error: '서버 내부 오류', code: 'DB_004' });
  }
};
